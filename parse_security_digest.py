...
class SecurityDigestParser:
    ...
    def _parse_category_news(self, category: str, content: str):
        ...
        current_news = {"category": category, "title": "", "content": "", "id": 0}
        news_started = False
...
def process_latest_article():
    """处理最新爬取的文章"""
    # 查找最新的文章目录
    article_dir = "wechat_articles"
    if not os.path.exists(article_dir):
        print(f"错误: 找不到文章目录 '{article_dir}'")
        return None

    # 查找最新的数据文件
    data_files = [f for f in os.listdir(article_dir) if f.endswith("_data.json")]
    if not data_files:
        print(f"错误: 在 '{article_dir}' 中找不到数据文件")
        return None

    # 按文件名排序（包含时间戳）
    latest_file = sorted(data_files)[-1]
    file_path = os.path.join(article_dir, latest_file)
    ...
    try:
        ...
        if "content" not in article_data or not article_data["content"]:
            print(f"错误: 数据文件 '{file_path}' 中没有内容")
            return None

        # 解析内容
        parser = SecurityDigestParser(article_data["content"])
        result = parser.parse()

        # 保存解析结果
        output_file = os.path.join(article_dir, "parsed_digest.json")
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)

        print(f"成功解析安全快报，结果已保存到 '{output_file}'")
        return result
...
if __name__ == "__main__":
    result = process_latest_article()
    if result:
        print(f"解析到 {result['news_count']} 条新闻")
        ...
        if result["news_items"]:
            sample = result["news_items"][0]
            print("\n示例新闻:")
            print(f"ID: {sample['id']}")
            print(f"分类: {sample['category']}")
            print(f"标题: {sample['title']}")
            print("内容预览: " + sample["content"][:100] + "...")
