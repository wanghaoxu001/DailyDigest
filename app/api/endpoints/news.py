...
def get_recent_news_grouped(
    ...
    # 过滤掉与已用于快报的新闻相似的文章（同一天的除外）
    filtered_news = similarity_service.filter_similar_to_used_news(all_news, db)
    
    groups = similarity_service.group_similar_news(filtered_news)
    ...
    for group in paginated_groups:
        ...
        for entity_type, entity_set in group['entities'].items():
            entities_dict[entity_type] = list(entity_set)
...
def get_recent_news_separated(
    ...
    # 使用新的分组方法分离今日文章
    grouped_news = similarity_service.group_todays_news_with_history_similar(all_items, db)
    
    fresh_news = grouped_news['fresh_news']
    similar_to_history = grouped_news['similar_to_history']
